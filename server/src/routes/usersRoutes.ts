...
router.get('/users', (_req, res) => {
 ...
 router.post('/users', (req, res) => {
    ...
    var user = new User(userList.counter, req.body.username, req.body.pass);
 ...
 router.get('/users/:id', (req, res) => {
    var user = userList.findUserById(req.params.id);
    if(user)
       res.end(JSON.stringify(user));
    else {
       res.statusCode = 400;
...
router.post('/users/:id/albums', (req, res) => {
   ...
   var album = new Album(albumList.counter, req.body.name);
